...
export function SessionDisplay({
  ...
  return (
    <>
      {!session.deleted && !viewDeleted && (
        <div className="flex items-center space-x-4 rounded-md border p-4">
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Users registered: {session.UserSession?.length || 0}
            </p>
            <p className="text-sm text-muted-foreground">
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      delete the session and remove all reservations to that
                      session.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      {viewDeleted && (
        <div
          className={`flex items-center space-x-4 rounded-md border p-4 ${session.deleted ? 'opacity-30' : ''}`}
        >
          <ImPushpin />
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium leading-none">{session.title}</p>
